...
```text
设计已闭合：是
实现已启动：是（SG-P0-LOCAL 已完成本地验证；SG-0～SG-3 LOCAL_VERIFIED，SG-4 MANIFEST_VERIFIED）
根 module：github.com/zhangzhe-ctrl/ani-session-gateway
API module：github.com/zhangzhe-ctrl/ani-session-gateway/api
...
1. `docs/design/realtime-session-gateway-p0-v1.2.md`：P0 权威设计、冻结技术基线、接口、安全不变量、批次和验收标准；v1.1 仅为历史版本。
...
- `SessionStore` 是真实 seam：Redis 是唯一生产/部署 Adapter；Memory 只用于 contract tests 和显式本地开发，二者运行同一 contract suite。禁止 Redis 失败自动降级到 Memory。
...
- HTTP Router：`net/http` + `github.com/go-chi/chi/v5`，整个进程只装配一个 `chi.Router`。
- WebSocket：`github.com/coder/websocket`。
- 内部控制接口：`google.golang.org/grpc`。
- Pod Exec：`k8s.io/client-go/tools/remotecommand`，隐藏在 `ExecRuntime` Adapter 后。
- KubeVirt：`kubevirt.io/client-go`，隐藏在 `VMConsoleRuntime` Adapter 后。
- Session Store：生产/部署固定 Redis；Memory 仅测试/显式本地开发。
- Metrics：`github.com/prometheus/client_golang`。
- Trace：OpenTelemetry Go；不得记录 ticket、query、credential、终端/VNC 内容或 ciphertext。
- 配置：环境变量 + 启动时强校验。不得静默替换依赖或自研 framework 包裹上述库。

## 4. 不可破坏的不变量

- ticket 至少 32 random bytes，只在 connect URL 中出现；日志、指标、错误、evidence 不得包含 ticket、query、credential 或会话内容。
- `CreateOrGet`、claim、容量 lease 和 close 语义以 v1.2 设计为准；Redis 多副本不能退化为进程内计数。
...
- Go module 根路径已由用户固定为 `github.com/zhangzhe-ctrl/ani-session-gateway`，不得自行迁移或改名。
- ANI 只依赖 `github.com/zhangzhe-ctrl/ani-session-gateway/api` 的固定版本；Git tag `api/v0.1.0` 在 ANI 的 `go.mod` 中对应 module version `v0.1.0`。禁止提交本机 `replace` 或复制 proto。
