# ANI Realtime Session Gateway P0 实施方案 v1.1
...
本节是 v1.1 的强制决策摘要；后文与本节冲突时以本节为准。
...
7. **claim 与容量**：Redis 模式下 ticket 比对、`issued -> claimed`、全局容量预占、subject 容量预占和租约写入必须在一次原子操作中完成；Memory 模式也通过同一 contract tests。关闭、过期和进程崩溃后的租约回收必须幂等，不能留下永久占用。
...
会话状态支持 Redis 和进程内存两种 Adapter：

- `redis`：Redis 不可用时启动失败，可多副本；
- `memory`：显式单副本，重启丢失未完成会话；
- `auto`：仅在启动时尝试 Redis，首次连接失败则固定退回内存，直到进程重启。

禁止进程运行中在 Redis 和内存之间热切换，避免已签发 ticket、幂等记录和 claim 状态分裂。
...
ani-session-gateway-ws:8080
  ...
  `- structured lifecycle logs + Prometheus metrics
```
...
- `CloseAndRelease` 必须幂等关闭并释放对应 lease；重复 close、连接异常、进程退出或 lease 超时不能造成负计数；Redis claim 前先清理已过期 lease，Memory Adapter 使用同一状态机语义；
...
这是真实 seam，因为 P0 确实存在 Redis 和 Memory 两个运行时 Adapter。`MemoryStore` 不是测试 fake。
...
| `STORE_MODE=memory` | `1` | 不连接 Redis | 不适用 |
| `STORE_MODE=auto` | `1` | 启动时退回 Memory | 若已选择 Redis则不热切换 |

`auto` 的选择只执行一次：

```text
process start
  |- Redis PING success -> RedisStore for entire process lifetime
  `- Redis PING failed  -> MemoryStore for entire process lifetime
```

不能在 Redis 中断时动态创建 MemoryStore。否则旧 ticket 在 Redis、新 ticket 在内存，不同副本对 claim 和幂等会得出不同结果。

RedisStore 的容量不是进程内计数。它使用 session lease 表达已 claim 的活动连接；`ClaimAndReserve` 在一个原子脚本中先删除 `expires_at <= now` 的 lease，再检查全局和 subject 数量，最后转换 session 状态并写入 lease。WebSocket 正常结束或异常结束调用 `CloseAndRelease`；进程崩溃时由 lease 的 session 最大时长自动回收。MemoryStore 复用同一状态机，但只允许单副本。
...
在 `redis` 模式下，Redis 运行中不可用时 readiness 返回失败；在 `auto` 已选择 memory 时 readiness 保持成功。
...
| `STORE_MODE` | `auto` | `redis|memory|auto` |
| `REDIS_URL` | 无 | redis/auto 使用 |
...
### SG-1：SessionManager + Redis/Memory Store
...
- 实现 `STORE_MODE=redis|memory|auto` 启动选择；
...
- Redis Secret 引用；
- NodePort 查重说明与部署前检查；
- `replicas: 1` 作为 auto/memory P0 默认。
...
- Pod restart 后服务恢复，Memory 模式未 claim session 丢失被明确观测。
...
7. ticket 重放失败；
8. ticket 过期失败；
9. Redis 启动失败时 auto/memory 单副本可用且 degraded 指标为 1；
...
contract:
- MemoryStore and RedisStore shared suite
...
2. 部署 Session Gateway，先验证 health、RBAC、Redis/Memory mode 和 NodePort；
